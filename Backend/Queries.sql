...
CREATE TABLE blog_post_comments(cid integer primary key auto_increment, postid integer references blog_posts(postid) on delete cascade on update set null, userid integer references blog_users(id) on delete cascade on update set null, comment text not null);